...
各种查询缓存策略的行为可以参考 [存储指南 - AVQuery 缓存查询](android_guide.html#缓存查询) 一节。
...
开发者需要注意的是，通过`AVIMClient.getConversationQuery()`这样得到的 `AVIMConversationQuery` 实例默认是查询全部对话的，也就是说，如果想查询指定的聊天室，需要额外再调用以 `where` 开头的方法来限定更多的查询条件：
